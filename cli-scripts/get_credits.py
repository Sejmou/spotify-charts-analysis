...
# def sync_main(track_ids: List[str]):
#     urls = [get_credits_api_url(track_id) for track_id in track_ids]
#     headers = get_internal_api_request_headers(track_ids[0])
...
if __name__ == "__main__":
    ...
    if os.path.exists(output_path):
        # load ids from the existing JSON objects in the file (extract from "track_uri" field)
        with open(output_path, "r") as f:
            existing_data = [json.loads(line) for line in f.readlines()]
            existing_track_ids = set(
                [obj["trackUri"].split(":")[2] for obj in existing_data]
            )
            print(
                    ...
                    if invalid_response_count == previous_invalid_response_count:
                        ...
                        with open(
                            os.path.join(
                                ...
                                f"missing_credits_responses.txt",
                            ),
                            "a",
                        ) as f:
                            for track_id in track_ids:
